<?php
    class Download{
        ...
        public function setLinkDownload($alvo){
            $this->linkdownload = $alvo;
        }
 
        public function downloaefetivo(){
            
            $content = file_get_contents( $this->getLinkdownload() );
            $parse = parse_url($this->getLinkDownload());
            $basename = basename($pase["path"]);
 
            if( $file = fopen($basename,"w+") ){
                fwrite($file, $content);
                fclose($file);
